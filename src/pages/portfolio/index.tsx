...
const Portfolio = () => {
  ...
  const [modalType, setModalType] = useState<"withdraw" | "deposit">("deposit")
  ...
  const columns = useMemo(() => {
    return [
            ...
            <HStack>
              {breakpoint !== "base" && breakpoint !== "sm" && (
                <IconButton
                  ...
                  bg="transparent"
                  color={
                    favourites.includes(info.row.original.token.token)
                      ? "yellow.600"
                      : "white"
                  }
                  _dark={{
                    color: favourites.includes(info.row.original.token.token)
                      ? "yellow.400"
                      : "white"
                  }}
                  icon={
                    favourites.includes(info.row.original.token.token) ? (
                      <Icon as={FaStar} w="1.5rem" h="1.5rem" />
                    ) : (
                      <Icon as={FaRegStar} w="1.5rem" h="1.5rem" />
                    )
                  }
                  onClick={() => addToFavourites(info.row.original.token.token)}
                />
              )}

              <HStack>
                <Skeleton
                  isLoaded={Boolean(info.row.original.token.imageUrl)}
                  rounded="full"
                >
                  <Avatar
                    size={{ base: "sm", md: "md" }}
                ...
                <Skeleton
                  isLoaded={Boolean(info.row.original.token.name)}
                  rounded="full"
                >
                  <VStack align="start">
                    <Text
                    ...
                    >
                      {
                        ChainConfigs[
                          TokenStatus[info.row.original.token.token].chain
                        ].chainName
                      }
                    </Text>
                  </VStack>
      ...
      columnHelper.accessor("balance", {
        ...
        cell: (info) => {
          return (
            <Skeleton isLoaded={Boolean(info.getValue())} rounded="full">
              <Text>
                {`${shortenNumber(
                  convertMicroDenomToDenom(
                    info.getValue(),
                    TokenStatus[info.row.original.token.token].decimal ?? 6
                  ),
                  2
                )}`}
              </Text>
